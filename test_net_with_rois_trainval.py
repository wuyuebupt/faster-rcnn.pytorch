...
def save_matfile(path, rois):
    assert(rois.size(0) == 1)
    rois = rois.view(rois.size(1), rois.size(2))
    # print (rois.cpu().data.numpy())
    sio.savemat(path, {'boxes': rois.cpu().data.numpy()})
    
   
def jpeg_to_mat(path):
    basename = os.path.basename(path)
    dirname = os.path.dirname(path)
    dirdirname = os.path.dirname(dirname)
    
    mat_basename = basename.split('.')[0] + '.mat'
    matfile = os.path.join(dirdirname, 'Proposals', mat_basename)
