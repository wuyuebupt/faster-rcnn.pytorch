...
def prepare_roidb(imdb):
  ...
  for i in range(len(imdb.image_index)):
    ...
    roidb[i]['flow'] = imdb.image_offline_flow_at(i)
...
def filter_roidb(roidb):
    ...
    i = 0
    while i < len(roidb):
      # if len(roidb[i]['boxes'][0]) == 0 or not os.path.exists(roidb[i]['offline_proposal'][0]) or len(roidb[i]['boxes'][0]) == 1 or not os.path.exists(roidb[i]['offline_proposal'][1]):
      if len(roidb[i]['boxes'][0]) == 0 or len(roidb[i]['boxes'][1]) == 0:
        del roidb[i]
        i -= 1
      i += 1
