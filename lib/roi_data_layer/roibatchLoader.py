...
class roibatchLoader(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if self.training:
        ...
        # image 2
        # shuffle is used for gt boxes and proposals -> 
        np.random.shuffle(blobs['gt_boxes'][1])
        gt_boxes_1 = torch.from_numpy(blobs['gt_boxes'][1])
        ...
        if self._roidb[index_ratio]['need_crop']:
            if ratio < 1:
                # this means that data_width << data_height, we need to crop the
                # data_height
                min_y = int(torch.min(gt_boxes_1[:,1]))
                max_y = int(torch.max(gt_boxes_1[:,3]))
                ...
                gt_boxes_1[:, 3] = gt_boxes_1[:, 3] - float(y_s)

                proposal_boxes_1[:, 1] = proposal_boxes_1[:, 1] - float(y_s)
                proposal_boxes_1[:, 3] = proposal_boxes_1[:, 3] - float(y_s)

                # update gt bounding box according the trip
                gt_boxes_1[:, 1].clamp_(0, trim_size - 1)
                gt_boxes_1[:, 3].clamp_(0, trim_size - 1)

                proposal_boxes_1[:, 1].clamp_(0, trim_size - 1)
                proposal_boxes_1[:, 3].clamp_(0, trim_size - 1)

            else:
                # this means that data_width >> data_height, we need to crop the
                # data_width
                min_x = int(torch.min(gt_boxes_1[:,0]))
                max_x = int(torch.max(gt_boxes_1[:,2]))
                trim_size = int(np.ceil(data_height * ratio))
                if trim_size > data_width:
                    trim_size = data_width                
                box_region = max_x - min_x + 1
                ...
                gt_boxes_1[:, 2] = gt_boxes_1[:, 2] - float(x_s)

                proposal_boxes_1[:, 0] = proposal_boxes_1[:, 0] - float(x_s)
                proposal_boxes_1[:, 2] = proposal_boxes_1[:, 2] - float(x_s)
                # update gt bounding box according the trip
                gt_boxes_1[:, 0].clamp_(0, trim_size - 1)
                gt_boxes_1[:, 2].clamp_(0, trim_size - 1)

                proposal_boxes_1[:, 0].clamp_(0, trim_size - 1)
                proposal_boxes_1[:, 2].clamp_(0, trim_size - 1)
        ...
        if ratio < 1:
            ...
            im_info_1[0, 0] = trim_size
            im_info_1[0, 1] = trim_size


        # check the bounding box:
        not_keep = (gt_boxes_1[:,0] == gt_boxes_1[:,2]) | (gt_boxes_1[:,1] == gt_boxes_1[:,3])
        keep = torch.nonzero(not_keep == 0).view(-1)

        gt_boxes_padding_1 = torch.FloatTensor(self.max_num_box, gt_boxes_1.size(1)).zero_()
        if keep.numel() != 0:
        ...
        # check the bounding box - proposal:
        not_keep_proposal = (proposal_boxes_1[:,0] == proposal_boxes_1[:,2]) | (proposal_boxes_1[:,1] == proposal_boxes_1[:,3])
        keep_proposal = torch.nonzero(not_keep_proposal == 0).view(-1)

        # proposal_boxes_padding = torch.FloatTensor(self.max_num_box, gt_boxes.size(1)).zero_()
        # mannual set the number of proposals to 2000
        proposal_boxes_padding_1 = torch.FloatTensor( 2000, proposal_boxes_1.size(1)).zero_()
        if keep_proposal.numel() != 0:
            proposal_boxes_1 = proposal_boxes_1[keep_proposal]
            num_proposals_1 = min(proposal_boxes_1.size(0), 2000)
            # print (proposal_boxes.size(), proposal_boxes.size(0), num_proposals)
            # swith the [bbox, score] to [~, bbox] to follow the rpn output format
