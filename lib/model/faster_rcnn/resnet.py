...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False):
    ...
    self.rcnn_model_path = 'data/pretrained_model/faster_rcnn_1_12_13649.pth'
  ...
  def _init_modules(self):
    ...
    # self.RCNN_cls_score = nn.Sequential(nn.Linear(2048, self.n_classes))
    if self.class_agnostic:
      self.RCNN_bbox_pred = nn.Linear(2048, 4)
    else:
      self.RCNN_bbox_pred = nn.Linear(2048, 4 * self.n_classes)

    # print (self.RCNN_cls_score.state_dict().keys())
    # print (self.RCNN_cls_score.state_dict())
    ## load after build
    if self.pretrained == True:
      print("Loading rcnn pre-train weights from %s" %(self.rcnn_model_path))
    ...
    self.tracking_concat_feat = nn.Sequential(
             nn.Conv2d(2 * self.dout_base_model + dimension_flow, self.dout_base_model , kernel_size=3, stride=1,padding=1, bias=False),
             nn.ReLU(inplace=True),
             nn.Conv2d(self.dout_base_model, self.dout_base_model , kernel_size=1, stride=1, bias=False),
             nn.ReLU(inplace=True) )
