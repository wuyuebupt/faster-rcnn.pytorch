...
def image_path_from_index(index):
  ...
  image_path = osp.join('data/coco/images/val2014', file_name)
  assert osp.exists(image_path), \
    'Path does not exist: {}'.format(image_path)
  return image_path
...
def write_images():
    ...
    print (len(image_ids))
    annos = _COCO.loadImgs(image_ids)
    print (len(annos))
    ...
    with h5py.File(output_h5, 'w') as f:
        ...
        for i, f in enumerate(image_ids):
            ...
            H, W, _ = img.shape
            print (img.shape)
            # if img.shape[0] ==  ann['height']:
            # if img.shape[0]+1 !=  ann['height']:
            #     print ('right')
            if img.shape[0] != ann['height'] or img.shape[1] != ann['width']:
                print (imgpath, f)
                exit()
            exit()
